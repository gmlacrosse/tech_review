...
namespace StargateAPI.Business.Commands.Tests
{
    ...
    public class CreatePersonTests
    {
        ...
        [TestMethod]
        public async Task CreatePersonHandlerTest()
        {
            using var context = new StargateContext(_options);
            var handler = new CreatePersonHandler(context);

            var result = await handler.Handle(new CreatePerson() { Name = "Unit Test" }, default);
        ...
        public async Task CreatePersonPreProcessorTest_PersonDoesNotExist()
        {
            ...
            var param = new CreatePerson() { Name = "Unit Test" };
        ...
        public async Task CreatePersonPreProcessorTest_PersonExists()
        {
            using (var context = new StargateContext(_options))
            {
                context.People.Add(new Person { Id = 1, Name = "Unit Test" });
